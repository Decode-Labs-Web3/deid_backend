...
class DecodeUserDataDTO(BaseModel):
    ...
    primary_wallet: Optional[DecodeWalletDataDTO] = Field(
        None, description="Primary wallet"
    )
    ...
    mutual_followers_number: Optional[int] = Field(
        None, description="Number of mutual followers"
    )
    mutual_followers_list: Optional[List[str]] = Field(
        None, description="List of mutual followers"
    )
...
class ProfileMetadataResponseDTO(BaseModel):
    ...
    statusCode: int = Field(..., description="HTTP status code")
    message: str = Field(..., description="Response message")
    data: Optional[ProfileMetadataDTO] = Field(
        None, description="Profile metadata from IPFS"
    )
    requestId: Optional[str] = Field(None, description="Request ID for tracing")
...
class UserSearchRequestDTO(BaseModel):
    ...
    email_or_username: str = Field(..., description="Email or username to search for")
    page: int = Field(0, ge=0, description="Page number (0-indexed)")
    limit: int = Field(100, ge=1, le=1000, description="Number of results per page")
