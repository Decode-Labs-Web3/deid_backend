...
async def get_discord_oauth_url(
    ...
    """
    try:
        oauth_url = await social_link_service.get_discord_oauth_url(current_user.user_id)
        return {
            "success": True,
            "oauth_url": oauth_url,
...
async def discord_oauth_callback(
    ...
    """
    logger.info(f"Discord OAuth callback received - code: {code[:10]}..., state: {state}")
...
async def confirm_onchain_verification(
    ...
    """
    logger.info(f"Onchain confirmation received - user_id: {current_user.user_id}, tx_hash: {request.tx_hash}, platform: {request.platform}, account_id: {request.account_id}")
...
async def get_user_social_links(
    ...
    """
    logger.info(f"Getting social links for user: {current_user.user_id}, status: {status}")

    try:
        social_links = await social_link_service.get_user_social_links(current_user.user_id, status)
...
async def get_user_social_link_stats(
    ...
    try:
        stats = await social_link_service.get_user_social_link_stats(current_user.user_id)
