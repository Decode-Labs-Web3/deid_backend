...
class SocialLinkModel(BaseModel):
    ...
    account_id: str = Field(..., description="Social account ID (e.g., Discord user ID)")
...
class SocialLinkCreateModel(BaseModel):
    ...
    signature: str = Field(..., description="EIP-712 signature for verification")
    verification_hash: str = Field(..., description="Hash used for verification")
    status: VerificationStatus = Field(default=VerificationStatus.VERIFIED, description="Verification status")


class SocialLinkUpdateModel(BaseModel):
    """Model for updating a social link."""

    status: Optional[VerificationStatus] = Field(None, description="Verification status")
    tx_hash: Optional[str] = Field(None, description="Transaction hash from smart contract")
    block_number: Optional[int] = Field(None, description="Block number of the transaction")
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Last update timestamp")
